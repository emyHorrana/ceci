...
// 2) Diagnóstico inicial: uma sequência de pequenas interações discretas
...
// 3) Ao final, os resultados ficam guardados no navegador (localStorage,
...
const DIAGNOSTIC_STEPS = [
  {
    ...
    instructions: 'Digite seu nome no campo abaixo e clique em Confirmar.',
    render: (reportResult) => <DigitarNomeGame reportResult={reportResult} />,
  },
  // Próximos passos possíveis: arrastar um cartão, clicar no botão
  // certo do mouse etc. - cada um vira só mais um item aqui.
];

export default function BoasVindas() {
  const navigate = useNavigate();
  const [fase, setFase] = useState('apresentacao'); // 'apresentacao' | 'diagnostico' | 'concluido'
    ...
    <div className={styles.page}>
      {fase === 'apresentacao' && (
        <div className={styles.card}>
          ...
          <ButtonPrimary size="large" onClick={() => setFase('diagnostico')}>
            Vamos lá!
          </ButtonPrimary>
        </div>
      )}
      ...
      {fase === 'concluido' && (
        <div className={styles.card}>
          ...
          <h1 className={styles.titulo}>
            Prazer, {onboarding?.nome?.meta?.nome || 'por aqui'}! 
          </h1>
          <p className={styles.texto}>
